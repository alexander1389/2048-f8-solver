...
class Calibrator:
    ...
    def manual_calibration(self, full_screen):
        ...
        if len(coords) == 4:
            ...
            self._board_region = (95, 251, 1806, 1874)
            print(f'Default coordinates are being used: {self._board_region}')

        print('\nEnter the tile size in the format: width height')
        print('This is the size of individual tiles on the game board.')
        tile_size = input('Tile size: ').strip().split()

        if len(tile_size) == 2:
            self._tile_width = int(tile_size[0])
            self._tile_height = int(tile_size[1])
            print(f'Tile size set: {self._tile_width}x{self._tile_height}')
        else:
            self._tile_width = 383
            self._tile_height = 383
            print(f'Default tile size is being used: {self._tile_width}x{self._tile_height}')
        ...
        return self._board_region is not None
    ...
    def analyze_tile_colors(self, cell_images):
        print('\n=== TILE COLOR ANALYSIS ===')
    ...
    def calibrate(self):
        ...
        self.analyze_tile_colors(cell_images)
